...
def _test_GRU(input_shape, bias,
                outer_iters=100, inner_iters=5, random_seed=None):
    if random_seed is not None:
        np.random.seed(random_seed)
    for _ in range(outer_iters):
        module1 = mm.GRU(input_shape[2], input_shape[2] * 2, bias = bias)
        module2 = nn.GRU(input_shape[2], input_shape[2] * 2, bias = bias, batch_first=True)
        for param1, param2 in zip(module1.parameters(), module2.parameters()):
            param2.data = torch.from_numpy(param1)
        debug_msg = f'Error in GRU '
        i = 0
